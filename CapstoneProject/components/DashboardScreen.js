...
const DashboardScreen = ({ navigation }) => {
 ...
 const renderSection = (title, data, keyPrefix, mediaType, isLast = false) => {
   if (!scrollRefs.current[title]) scrollRefs.current[title] = React.createRef();


   return (
     <View style={[styles.sectionContainer, !isLast && styles.sectionSpacing]}>
       ...
       <View style={styles.carouselRow}>
         {isWeb && (
           <TouchableOpacity
             ...
             onPress={() =>
               scrollRefs.current[title]?.current?.scrollToOffset({
                 offset: 0,
                 animated: true,
               })
             }
         ...
         {isWeb && (
           <TouchableOpacity
             style={styles.arrowButton}
             onPress={() =>
               scrollRefs.current[title]?.current?.scrollToEnd({ animated: true })
             }
           >
             <Text style={styles.arrowIcon}>▶</Text>
           </TouchableOpacity>
